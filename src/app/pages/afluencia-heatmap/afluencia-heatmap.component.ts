  ...
  export class AfluenciaHeatmapComponent implements OnInit {
    ...
    parqueaderosDisponibles: { parkingId: string, nombre: string, capacidad: number, horarioAtencion:string }[] = [];
    ...
    ngOnInit(): void {
      ...
      this.api.obtenerAfluencia('2025-06-29','2025-07-06').subscribe((res: RegistroAfluencia[]) => {
    ...
    extraerRangoHorario(horarioAtencion: string): { inicio: number, fin: number } {
      const partes = horarioAtencion.split('a').map(p => p.trim());
      const inicio = +partes[0].split(':')[0];
      const fin = +partes[1].split(':')[0];
      return { inicio, fin };
    }


  actualizarTabla(datos: any[]) {
    // 1. Define horario por defecto (si es 'Todos' o parqueadero no encontrado)
    let horario_inicio = 6, horario_fin = 23;
    if (this.parqueaderoSeleccionado !== 'Todos') {
      const parqueadero = this.parqueaderosDisponibles.find(
        p => p.nombre === this.parqueaderoSeleccionado
      );
      if (parqueadero && parqueadero.horarioAtencion) {
        // Asume formato "06:00 a 23:00"
        const partes = parqueadero.horarioAtencion.split('a').map(s => s.trim());
        horario_inicio = +partes[0].split(':')[0];
        horario_fin = +partes[1].split(':')[0];
      }
    }
    ...
    // Filtra solo las horas del horario de atención
    ...
    if (datos.length === 0) {
      // Solo inicializa matriz vacía para ese rango horario
    ...
    // Si hay datos, llena solo las horas que están en el rango
    datos.forEach(r => {
    ...
    const diasMap = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
